...
In AsciiDoc, literal blocks can be delimited (created) with `----` or pass:[`****`]. So if you need to have something like:
...
AsciiDoc will recognize the pass:[`****`] as an opening of a literal block. 

The solution to this is to add something after the pass:[`****`]. This uses the builtin pass:q[`{empty}`]:
